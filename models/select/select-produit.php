<?php
    if (isset($_GET['idProd'])){
     $id=$_GET['idProd'];
     $getDataMod=$connexion->prepare("SELECT * FROM produits WHERE id= ?");
     $getDataMod->execute([$id]);
     $tab=$getDataMod->fetch();
        
     $url="../models/updat/up-produit-post.php?idProd=".$id;//Cette varible permet de savoir sur quelle page l'action va etre executée lors de la modification
    ...
    if(isset($_GET['search']) && !empty($_GET['search'])){
        ...
        $getData = $connexion->prepare("SELECT * from produits WHERE statut=?");
        $getData->execute([0]);
        // $getData=$connexion->prepare("SELECT  catproduit.`description`, typeproduit.description,catproduit.id  FROM typeproduit,`catproduit` 
